...
class DrugPatientDatasetTCGA(Dataset):
    """
    用于TCGA药物-患者组合的数据集类，支持单一和组合药物治疗，包括PPI矩阵和编码张量
    """
    ...
    def __init__(self, drug_file, patient_embed_file, sensitivity_file,  ppi_file,
        ...
        merged_data = merged_data.dropna(subset=['SMILES', 'afm', 'adj'])
        merged_data.to_csv('tcga_filtered_data.csv', index=False)
